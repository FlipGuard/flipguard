...
export const ExtensionEditDialog = ({ open, handleClose, extensionId, onUpdate, isUpdateLoading }: Props) => {
    const { user } = useAuth();
    ...
    const affectedBots = Object.values(user.metadata.bots)
        .filter((bot) => bot.usedExtensions.includes(extensionId) && bot.active)
        .map((bot) => ({ id: bot.id, name: bot.name }));

    return (
        <ConfirmDialogBase isOpen={open} onClose={handleClose} title={`Update "${extensionId}"?`}>
            <DialogContent sx={{ padding: '12px' }}>
                <DialogContentText>
                    You will have to restart already running bots that use this extension for the change to occur.
                </DialogContentText>
