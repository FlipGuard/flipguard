...
export const IntegrationEditDialog = ({ open, handleClose, integrationId, onUpdate, isUpdateLoading }: Props) => {
    const { user } = useAuth();
    ...
    const affectedBots = Object.values(user.metadata.bots)
        .filter((bot) => bot.usedIntegrations.includes(integrationId) && bot.active)
        .map((bot) => ({ id: bot.id, name: bot.name }));

    return (
        <ConfirmDialogBase isOpen={open} onClose={handleClose} title={`Update "${integrationId}"?`}>
            <DialogContent sx={{ padding: '12px' }}>
                <DialogContentText>
                    You will have to restart already running bots that use this integration for the change to occur.
                </DialogContentText>
